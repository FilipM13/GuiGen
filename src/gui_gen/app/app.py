...
class App(metaclass=MetaTemplated):
    ...
    def build(self):
        ...
        for template in MetaTemplated.all_templates:
            # copy html template
            os.makedirs(os.path.dirname(self.gui_directory + template.template_html), exist_ok=True)
            shutil.copy(template.abs_template_html, self.gui_directory + template.template_html)
